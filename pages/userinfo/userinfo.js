...
Page({
  ...
  formSubmit: function (res) {
    ...
    //验证
    if (!data.username) {
      mythis.showmsg('请填写姓名');
      return;
    }

    if (!data.borndate || data.borndate == '0000-00-00') {
      mythis.showmsg('请选择出生年月');
      return;
    }
    if (!data.worktime) {
      mythis.showmsg('请选择工作年限');
      return;
    }
    if (!data.city) {
      mythis.showmsg('请选择所在城市');
      return;
    }
    if (!data.mobile) {
      mythis.showmsg('请输入联系方式');
      return;
    }

    if (!app.globalData.mobile.test(data.mobile)) {
      mythis.showmsg('手机格式错误');
      return;
    }

    if (data.introduction.length < 10) {
      mythis.showmsg('简介不能小于10字符')
      return;
    }
    if (data.sex == '男') {
      data.sex = 1;
